...
path_to_f<-"/home/forbes.ai/capstone_dir/Seurat/diff_exp_reclustered_relaxed.rds"
...
cds_p125 <- cds[, partitions(cds) %in% c("1", "2", "5")]
cds_p125 <-preprocess_cds(cds_p125, num_dim = 50)
plot_pc_variance_explained(cds_p125)
cds_p125 <-reduce_dimension(cds_p125)
cds_p125 <- cluster_cells(cds_p125, partition_qval = 1)


part_cluster_plot_2<-plot_cells(cds_p23, color_cells_by = "partition",
           graph_label_size=1.5, label_cell_groups =FALSE)
part_cluster_plot_2 + ggtitle("part_cluster_plot_2")


part_cluster_plot_3<-plot_cells(cds_p125, color_cells_by = "partition",
           graph_label_size=1.5, label_cell_groups =FALSE)
part_cluster_plot_3 + ggtitle("part_cluster_plot_3")

cds <- learn_graph(cds)
cds_p125 <- learn_graph(cds_p125)
...
seur_cluster_plot_2<-plot_cells(cds_p125,
...
#root 1 for relaxed
root_cells <- colnames(cds)[cds$seurat_clusters == "1"]
...
puesdotime_cluster_plot_2<-plot_cells(cds_p125,
           color_cells_by = "pseudotime",
           label_cell_groups=FALSE,
           label_leaves=FALSE,
           label_branch_points=FALSE,
